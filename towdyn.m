function [X,Y,Z,iobj]=towdyn(U,z,Ht,Bt,Cdt,MEt,V,W,rho,Usp,Vsp)
...
% add ship speed to current profile
U=U-Usp;
V=V-Vsp;
%
[mu,nu]=size(U);
...
if (uw^2+vw^2)>0,
   ...
   U(uwindx)=U(uwindx)+uw1;
   V(uwindx)=V(uwindx)+vw1;
end
end
...
while breaknow==0,  % loop through a few times to converge solution.
   ...
   %
   psi(2)=atan2(sqrt(Qx(1)^2+Qy(1)^2),b);  % angle/tilt to the second element (of wire)
   psi(1)=0; % angle below bottom device is zero (undefined)
